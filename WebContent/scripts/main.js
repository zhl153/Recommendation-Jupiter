(function() {
   ...
   */
  function init() {
    // register event listeners //给页面元素添加methods
   ...
   * Session
   */
  function validateSession() { // 检查是否login
    onSessionInvalid(); // 默认logout
    ...
    // make AJAX call
    ajax('GET', url, req, // ajax->line 336
      // session is still valid
      function(res) { // 收到response
        var result = JSON.parse(res);

        if (result.status === 'OK') {
          onSessionValid(result); // 验证login，则切换状态
        }
      });
  }

  function onSessionValid(result) { // 已登录则显示以下页面
  ...
  function onSessionInvalid() { // 未登录， 隐藏掉一些东西
    var loginForm = document.querySelector('#login-form');
    var registerForm = document.querySelector('#register-form');
    var itemNav = document.querySelector('#item-nav');
    var itemList = document.querySelector('#item-list');
  ...
  function showRegisterForm() { // show the register form and hide other elements
    var loginForm = document.querySelector('#login-form');
    var registerForm = document.querySelector('#register-form');
    var itemNav = document.querySelector('#item-nav');
    var itemList = document.querySelector('#item-list');
  ...
  function login() {
    ...
    ajax('POST', url, req,
      ...
      function(res) {
        ...
        if (result.status === 'OK') {
          onSessionValid(result); // show content
        }
      },
  ...
  function showLoginError() {
    document.querySelector('#login-error').innerHTML = 'Invalid username or password'; // insert txt into the p tag
  }
   ...
   * @param data - request payload data (payload = request body)
   * @param successCallback - Successful callback function
   * @param errorCallback - Error callback function
   */
  function ajax(method, url, data, successCallback, errorCallback) { // ajax helper函数
    ...
    if (data === null) { 
      ...
      xhr.setRequestHeader("Content-Type", // property name: url encoding
        "application/json;charset=utf-8"); // property value: encoding method and data type
      xhr.send(data);
    }
  }
  ...
  function changeFavoriteItem(item_id) {
    ...
    ajax(method, url, req, // 进行ajax call
      // successful callback
      function(res) {
        var result = JSON.parse(res);
